# 2026-07-31

## 活动介绍

![zhangshanyou](../../images/speakers/zhangshanyou.jpg)

讲师：**张善友**

广州智用开物人工智能科技有限公司 CTO、微软最有价值专家(MVP)

广州智用开物人工智能科技有限公司 CTO、微软最有价值专家(MVP)、腾讯云 TVP、华为云 HCDE

主题：**AI Builder Talks｜构建面向 .NET 的 NativeAOT 友好的智能体运行时**

聚焦 OpenClaw.NET 这一 .NET 生态最新实现，深入解析 OpenClaw.NET 的整体架构与设计理念，围绕 NativeAOT、智能体运行时、多模型 Provider、MCP 工具生态、OpenTelemetry 可观测性及生产部署等内容展开，并通过现场 Demo 演示如何快速构建一个高性能、可观测、可自托管的 AI Agent 系统，为 .NET 开发者带来从架构到实践的全面解析。

## 标签

- .NET AI
- AI
- Agents
- OpenClaw
- OpenClaw.NET

## 更多

- [活动介绍](https://developer.microsoft.com/zh-cn/reactor/events/27474)
- [视频回放](https://www.bilibili.com/video/BV1wRGA6jEcC/)
- [讲师 Github](https://github.com/geffzhang)
- [PPT](./Build-dotnet-oriented-NativeAOT-Friendly-AI-Agent-Runtime.pdf)
- [OpenClaw.NET GitHub 仓库](https://github.com/clawdotnet/openclaw.net)
- [博客园博客](https://www.cnblogs.com/shanyou)
